...
/// activation step. In production `gohd` binds its Mach service with an
...
///
/// An XPC session is active as soon as it is created. The peer requirement is
/// supplied at construction so the daemon is validated before any message is
/// exchanged (see `DESIGN.md` §3.2).
